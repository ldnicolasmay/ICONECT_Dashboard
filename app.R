...
server <- function(input, output, session) {
  ...
  recruit_status_summ_list <- reactive({
    list(recruit_status_summ_mi = recruit_status_summ_mi(),
         recruit_status_summ_or = recruit_status_summ_or())
  })
  observe({
    lapply(site_names, # site_name <- c('mi', 'or')
           function(site_name) {
             site_name_p = paste0('recruit_status_summ_', site_name)
             output[[paste0(site_name_p, '_tbl')]] <-
               renderDataTable({
                 recruit_status_summ_list()[[site_name_p]] %>%
  ...
  # output$recruit_status_summ_mi_tbl <- renderDataTable({
  ...
  # })
  #
  # output$recruit_status_summ_or_tbl <- renderDataTable({
  ...
  recruit_lead_summ_list <- reactive({
    list(recruit_lead_summ_mi = recruit_lead_summ_mi(),
         recruit_lead_summ_or = recruit_lead_summ_or())
  })
  observe({
    lapply(site_names, # site_name <- c('mi', 'or')
           function(site_name) {
             site_name_p = paste0('recruit_lead_summ_', site_name)
             output[[paste0(site_name_p, '_tbl')]] <-
               renderDataTable({
                 recruit_lead_summ_list()[[site_name_p]] %>%
  ...
  # output$recruit_lead_summ_mi_tbl <- renderDataTable({
  ...
  # })
  #
  # output$recruit_lead_summ_or_tbl <- renderDataTable({
  ...
  telscrn_elg_summ_list <- reactive({
    list(telscrn_elg_summ_mi = telscrn_elg_summ_mi(),
         telscrn_elg_summ_or = telscrn_elg_summ_or())
  })
  observe({
    lapply(site_names, # site_name <- c('mi', 'or')
           function(site_name) {
             site_name_p = paste0('telscrn_elg_summ_', site_name)
             output[[paste0(site_name_p, '_tbl')]] <-
               renderDataTable({
                 telscrn_elg_summ_list()[[site_name_p]] %>%
  ...
  # output$telscrn_elg_summ_mi_tbl <- renderDataTable({
  #   telscrn_elg_summ_mi() %>%
  #     rename(`Eligibilty Status` = ts_elg_txt,
  #            `Prop` = proportion) %>%
  #     datatable(rownames = FALSE, options = DT_OPTIONS) %>%
  ...
  # })
  #
  # output$telscrn_elg_summ_or_tbl <- renderDataTable({
  #   telscrn_elg_summ_or() %>%
  #     rename(`Eligibilty Status` = ts_elg_txt,
  #            `Prop` = proportion) %>%
  #     datatable(rownames = FALSE, options = DT_OPTIONS) %>%
  ...
  telscrn_en_summ_list <- reactive({
    list(telscrn_en_summ_mi = telscrn_en_summ_mi(),
         telscrn_en_summ_or = telscrn_en_summ_or())
  })
  observe({
    lapply(site_names, # site_name <- c('mi', 'or')
           function(site_name) {
             site_name_p = paste0('telscrn_en_summ_', site_name)
             output[[paste0(site_name_p, '_tbl')]] <-
               renderDataTable({
                 telscrn_en_summ_list()[[site_name_p]] %>%
  ...
  # output$telscrn_en_summ_mi_tbl <- renderDataTable({
  #   telscrn_en_summ_mi() %>%
  #     rename(`Ineligibility Reason` = ts_en_txt,
  #            `Prop` = proportion) %>%
  #     datatable(rownames = FALSE, options = DT_OPTIONS) %>%
  ...
  # })
  #
  # output$telscrn_en_summ_or_tbl <- renderDataTable({
  #   telscrn_en_summ_or() %>%
  #     rename(`Ineligibility Reason` = ts_en_txt,
  #            `Prop` = proportion) %>%
  #     datatable(rownames = FALSE, options = DT_OPTIONS) %>%
